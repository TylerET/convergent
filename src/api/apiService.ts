...
export const deleteEvent = async (userId: number) => {
  const apiUrl = `${basePath}/events/${userId}`;
  try {
    const response = await fetch(apiUrl, {
      method: "DELETE",
      headers: {
    ...
    console.error("Error in delete event API call:", error.message);
  }
};
...
export const getAllEventsByHostedById = async (userId: number) => {
  const apiUrl = `${basePath}/events/hosted/${userId}`;
  try {
    const response = await fetch(apiUrl, {
      method: "GET",
      headers: {
